...
Три модели Cloud Run протестированы по всем 12 вопросам с keyword + semantic (BAAI/bge-base-en-v1.5) + hybrid скорингом. Оптимизатор Qwen2.5:7b был включён; `↑` — вопросы, в которых оптимизатор восстановил базовую оценку 0.
...
| **Итого** | | **66.7% / 68.8% / 60.4%** | **62.5% / 64.6% / 54.2%** | **62.5% / 62.5% / 52.1%** |
| **Вердикт** | | requires-validation | requires-validation | requires-validation |

> **bold** = 100 (технически точный ответ) · ↑ = оптимизатор Qwen2.5:7b восстановил оценку (стратегия role_playing) · ~~зачёркнуто~~ = оценка до оптимизации · Q5 Tongyi — результат изолированного повтора после временного сбоя соединения Cloud Run в полном прогоне

### Пример вывода — DeepSeek-R1 8B (Cloud Run, BGE semantic + оптимизатор)
