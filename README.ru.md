...
| **Итого** | | **66.7% / 68.8% / 60.4%** | **62.5% / 64.6% / 54.2%** | **58.3% / 58.3% / 47.9%** |
| **Вердикт** | | requires-validation | requires-validation | not-suitable |

> **bold** = 100 (технически точный ответ) · ↑ = оптимизатор Qwen2.5:7b восстановил оценку (стратегия role_playing) · ~~зачёркнуто~~ = оценка до оптимизации · *conn. error* = временный сбой соединения Cloud Run, не отказ модели
