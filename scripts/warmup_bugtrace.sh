#!/usr/bin/env bash
# Warm up BugTrace on Cloud Run (cold-start ping). Plain chat ping — no Ollama keep_alive on target.
set -euo pipefail
...
echo "   model:    $BUGTRACE_MODEL"
echo "   timeout:  ${BUGTRACE_PING_TIMEOUT_S}s"

TOKEN="$(cloudrun_token)"
if ! resp="$(ping_ollama_chat \
  "$BUGTRACE_ENDPOINT" "$BUGTRACE_MODEL" "$TOKEN" "" \
  "$PING_PROMPT" "$PING_MAX_TOKENS" "$BUGTRACE_PING_TIMEOUT_S")"; then
  print_ping_fail "target"
  die "BugTrace warmup failed (timeout ${BUGTRACE_PING_TIMEOUT_S}s or HTTP error)"
fi
